define([
	...
	//GTI: copied from build/transforms/writeAmd, and added replacing locale in GNLS resources' mids
	var
		computingLayers
		...
		getLayerText = function(
			...
			for(var p in moduleSet){
				...
				if(p!=resource.mid || resourceText===false){
					...
					if(module.localizedSet && bc.localeList){
						...
						//GTI
						cache.push("'" + p.replace("$$LOCALE$$",resource.layer.locale) + "':function(){" + newline + module.getText() + newline + "}");
					}else{
			...
			return	(cache.length ? "require({cache:{" + newline + cache.join("," + newline) + "}});" + newline : "") +
				(resourceText===undefined ?	 insertAbsMid(resource.getText(), resource) : (!resourceText ? "" : resourceText)) +
		...
		processNlsBundle = function(
			...
			if(resource.localizedSet && resource.bundleValue){
				...
				if(missing.length){
					missing.sort();/* git-qa */ //array of strings
